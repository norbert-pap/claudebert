...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/claudebert-cli-');
    const execPath = path.join(root, '.claude/skills/claudebert/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/claudebert/browse/src/server.ts');
