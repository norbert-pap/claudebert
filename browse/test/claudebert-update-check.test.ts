/**
 * Tests for bin/claudebert-update-check bash script.
 *
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'claudebert-update-check');

let claudebertDir: string;
let stateDir: string;
...
function run(extraEnv: Record<string, string> = {}, args: string[] = []) {
  const result = Bun.spawnSync(['bash', SCRIPT, ...args], {
    env: {
      ...process.env,
      GSTACK_DIR: claudebertDir,
      GSTACK_STATE_DIR: stateDir,
      GSTACK_REMOTE_URL: `file://${join(claudebertDir, 'REMOTE_VERSION')}`,
      ...extraEnv,
    },
    stdout: 'pipe',
    stderr: 'pipe',
...
beforeEach(() => {
  claudebertDir = mkdtempSync(join(tmpdir(), 'claudebert-upd-test-'));
  stateDir = mkdtempSync(join(tmpdir(), 'claudebert-state-test-'));
  // Link real claudebert-config so update_check config check works
  const binDir = join(claudebertDir, 'bin');
  mkdirSync(binDir);
  symlinkSync(join(import.meta.dir, '..', '..', 'bin', 'claudebert-config'), join(binDir, 'claudebert-config'));
});

afterEach(() => {
  rmSync(claudebertDir, { recursive: true, force: true });
...
describe('claudebert-update-check', () => {
  ...
  test('exits 0 with no output when VERSION file is empty', () => {
    writeFileSync(join(claudebertDir, 'VERSION'), '');
    const { exitCode, stdout } = run();
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
  });
  ...
  test('outputs JUST_UPGRADED and deletes marker', () => {
    writeFileSync(join(claudebertDir, 'VERSION'), '0.4.0\n');
  ...
  test('exits silently when cache says UP_TO_DATE and is fresh', () => {
    writeFileSync(join(claudebertDir, 'VERSION'), '0.3.3\n');
  ...
  test('re-checks when UP_TO_DATE cache version does not match local', () => {
    ...
    writeFileSync(join(claudebertDir, 'REMOTE_VERSION'), '0.5.0\n');
  ...
  test('re-checks when local version does not match cached old version', () => {
    ...
    writeFileSync(join(claudebertDir, 'REMOTE_VERSION'), '0.4.0\n');
  ...
  test('writes UP_TO_DATE cache when versions match', () => {
    writeFileSync(join(claudebertDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(claudebertDir, 'REMOTE_VERSION'), '0.3.3\n');
  ...
  test('treats invalid remote response as up to date', () => {
    writeFileSync(join(claudebertDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(claudebertDir, 'REMOTE_VERSION'), '<html>404 Not Found</html>\n');
  ...
  test('exits 0 with real project VERSION and unreachable remote', () => {
    ...
    writeFileSync(join(claudebertDir, 'VERSION'), version + '\n');
