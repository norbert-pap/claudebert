...
allowed-tools:
  ...
  - AskUserQuestion
---
<!-- claudebert/plan-ceo-review — forked from claudebert, made framework-agnostic -->
...
Critical rule: Once the user selects a mode, COMMIT to it. Do not silently drift toward a different mode. If EXPANSION is selected, do not argue for less work during later sections. If REDUCTION is selected, do not sneak scope back in. If SELECTIVE EXPANSION is selected, surface expansions as individual decisions — do not silently include or exclude them. Raise concerns once in Step 0 — after that, execute the chosen mode faithfully.
Do NOT make any code changes. Do NOT start implementation. Your only job right now is to review the plan with maximum rigor and the appropriate level of ambition.

## Prime Directives
1. Zero silent failures. Every failure mode must be visible — to the system, to the team, to the user. If a failure can happen silently, that is a critical defect in the plan.
2. Every error has a name. Don't say "handle errors." Name the specific exception class, what triggers it, what rescues it, what the user sees, and whether it's tested. catching generic exceptions (e.g. catch(Exception), rescue StandardError, except Exception) is a code smell — call it out.
...
* ASCII diagrams in code comments for complex designs — Models (state transitions), Services (pipelines), Routes/Controllers (request flow), Middleware/Mixins (shared behavior), Tests (non-obvious setup).
...
grep -r "TODO\|FIXME\|HACK\|XXX" --include="*.ts" --include="*.tsx" --include="*.js" --include="*.jsx" --include="*.py" --include="*.rb" -l
git diff --stat HEAD~10  # Recently touched files
```
...
mkdir -p ~/.claudebert/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.claudebert/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv to archive/
```

Write to `~/.claudebert/projects/{slug}/ceo-plans/{date}-{feature-slug}.md` using this format:
...
Generated by /prod-review on {date}
...
2. **SELECTIVE EXPANSION:** Hold the current scope as baseline, but surface every expansion opportunity individually. Cherry-pick what's worth adding.
3. **HOLD SCOPE:** The plan's scope is right. Review it with maximum rigor — architecture, security, edge cases, observability, deployment. Make it bulletproof.
...
* Rollback posture. If this ships and immediately breaks, what's the rollback procedure? Git revert? Feature flag? database migration rollback? How long?

**EXPANSION and SELECTIVE EXPANSION mode additions:**
...
```
  ...
  -------------------------|-----------------------------|-----------------
  ExampleService#call      | API timeout                 | HTTP timeout
                           | API returns 429             | Rate limit error
                           | API returns malformed JSON  | JSON parse error
                           | DB connection pool exhausted| DB connection error
                           | Record not found            | RecordNotFound
  -------------------------|-----------------------------|-----------------

  EXCEPTION CLASS              | RESCUED?  | RESCUE ACTION          | USER SEES
  -----------------------------|-----------|------------------------|------------------
  HTTP timeout                 | Y         | Retry 2x, then raise   | "Service temporarily unavailable"
  Rate limit error             | Y         | Backoff + retry         | Nothing (transparent)
  JSON parse error             | N ← GAP   | —                      | 500 error ← BAD
  DB connection error          | N ← GAP   | —                      | 500 error ← BAD
  RecordNotFound               | Y         | Return nil, log warning | "Not found" message
```
Rules for this section:
* `catching generic exceptions (e.g. catch(Exception), rescue StandardError, except Exception)` is ALWAYS a smell. Name the specific exceptions.
* `generic error catching` with only `logger.error(e.message)` is insufficient. Log the full context: what was being attempted, with what arguments, for what user/request.
...
* Dependency risk. New dependencies? Security track record?
...
Evaluate:
* N+1 queries. For every database query inside a loop: is there batching or eager loading?
...
* Admin tooling. New operational tasks that need admin UI or CLI tasks/scripts?
* Runbooks. For each new failure mode: what's the operational response?

**EXPANSION and SELECTIVE EXPANSION mode addition:**
* What observability would make this feature a joy to operate?
**STOP.** AskUserQuestion once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.

### Section 9: Deployment & Rollout Review
Evaluate:
* Migration safety. For every new database migration: backward-compatible? Zero-downtime? Table locks?
...
* Ecosystem fit. Aligns with project's technology ecosystem direction?
...
For deep design work, recommend `/design-review` after implementation.
**STOP.** AskUserQuestion once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.
...
```
  ...
  +====================================================================+
  | Mode selected        | EXPANSION / SELECTIVE EXPANSION /           |
  |                      | HOLD / REDUCTION                            |
  ...
  | Section 11 (Design)  | ___ issues found (or N/A if no UI scope)    |
  +--------------------------------------------------------------------+
...
```
  ...
  ├─────────────┼──────────────┼──────────────────┼──────────────┼──────────────┤
  │ Scope       │ Push UP      │ Hold + cherry-   │ Maintain     │ Push DOWN    │
  │             │              │ pick expansions  │              │              │
  │ 10x check   │ Mandatory    │ Mandatory        │ Optional     │ Skip         │
  │ Platonic    │ Yes          │ No               │ No           │ No           │
  │ ideal       │              │                  │              │              │
  │ Delight     │ 5+ items     │ 5+ candidates    │ Note if seen │ Skip         │
  │ opps        │              │                  │              │              │
  │ Complexity  │ "Is it big   │ "Is it tight     │ "Is it too   │ "Is it the   │
  │ question    │  enough?"    │  enough + what   │  complex?"   │  bare min?"  │
  │             │              │  to cherry-pick?"│              │              │
  │ Taste       │ Yes          │ Yes              │ No           │ No           │
  │ calibration │              │                  │              │              │
  │ Temporal    │ Full (hr 1-6)│ Full (hr 1-6)   │ Key decisions│ Skip         │
  │ interrogate │              │                  │  only        │              │
  │ Observ.     │ "Joy to      │ "Joy to          │ "Can we      │ "Can we see  │
  │ standard    │  operate"    │  operate"        │  debug it?"  │  if broken?" │
  │ Deploy      │ Infra as     │ Infra as         │ Safe deploy  │ Simplest     │
  │ standard    │ feature scope│ feature scope    │  + rollback  │  possible    │
  │ Error map   │ Full + chaos │ Full + chaos     │ Full         │ Critical     │
  │             │  scenarios   │  scenarios       │              │  paths only  │
  │ Phase 2/3   │ Map it       │ Map it           │ Note it      │ Skip         │
  │ planning    │              │                  │              │              │
  │ CEO Plan    │ Persisted    │ Persisted        │ No           │ No           │
  │ persistence │              │                  │              │              │
  └─────────────┴──────────────┴──────────────────┴──────────────┴──────────────┘
```
